...
class Atis_Intent_Detection_Processor(DataProcessor):
    """Processor for the atis_Intent_Detection_and_Slot_Filling data set"""
    # atis data from https://github.com/MiuLab/SlotGated-SLU/tree/master/data/atis
    def get_examples(self, data_dir):
        with open(os.path.join(data_dir, "seq.in")) as seq_in_f:
            with open(os.path.join(data_dir, "label")) as label_f:
                ...
                intent_label_list = [seq.replace("\n", '') for seq in label_f.readlines()]
                assert len(seq_in_list) == len(intent_label_list)
                examples = list(zip(seq_in_list, intent_label_list))
                return examples
...
class Snips_Intent_Detection_Processor(DataProcessor):
    """Processor for the snip_Intent_Detection_and_Slot_Filling data set"""
    # snips data from https://github.com/MiuLab/SlotGated-SLU/tree/master/data/snips
    def get_examples(self, data_dir):
        with open(os.path.join(data_dir, "seq.in")) as seq_in_f:
            with open(os.path.join(data_dir, "label")) as label_f:
                seq_in_list = [seq.replace("\n", '') for seq in seq_in_f.readlines()]
...
class SMP_Task1_Intent_Detection_Processor(DataProcessor):
    """Processor for the SMP_Task1_Intent_Detection_and_Slot_Filling data set"""
    def get_examples(self, data_dir):
        with open(os.path.join(data_dir, "seq.in")) as seq_in_f:
            with open(os.path.join(data_dir, "label")) as label_f:
                seq_in_list = [seq.replace("\n", '') for seq in seq_in_f.readlines()]
    ...
    def get_labels(self):
        return ['LAUNCH', 'QUERY', 'ROUTE', 'SENDCONTACTS', 'SEND', 'REPLY', 'REPLAY_ALL', 'LOOK_BACK',
                'NUMBER_QUERY', 'POSITION', 'PLAY', 'DEFAULT', 'DIAL', 'TRANSLATION', 'OPEN', 'CREATE',
                'FORWARD', 'VIEW', 'SEARCH', 'RISERATE_QUERY', 'DOWNLOAD', 'DATE_QUERY', 'CLOSEPRICE_QUERY']
...
def main(_):
    ...
    processors = {
        ...
        "mrpc": MrpcProcessor,
        "xnli": XnliProcessor,
        "atis": Atis_Intent_Detection_Processor,
        "snips": Snips_Intent_Detection_Processor,
        "smp1": SMP_Task1_Intent_Detection_Processor
    }
    ...
    if FLAGS.do_predict:
        ...
        output_predict_file = os.path.join(FLAGS.output_dir, "intent_prediction_test_results.txt")
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            num_written_lines = 0
            tf.logging.info("***** Predict results *****")
            for (i, prediction) in enumerate(result):
                intent_prediction = prediction["predictions"]
                if i >= num_actual_predict_examples:
                    break
                intent_output_line = str(intent_id2label[intent_prediction]) + "\n"
                writer.write(intent_output_line)
                num_written_lines += 1
